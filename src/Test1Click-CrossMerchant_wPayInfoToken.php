...
$request->Set(GatewayRequest::EMAIL(), "1clicktest@fakedomain.com");
...
if ($service->PerformPurchase($request, $response)) {
  ...
  print "GUID: " . $response->Get(GatewayResponse::TRANSACT_ID()) . "\n";
	...
	// Run additional purchase using PAYINFO_TRANSACT_ID
	//
	//  This would normally be two separate processes, 
	//  but for example's sake is in one process (thus we clear and set a new GatewayRequest object)
	//  The key values required are MERCHANT_CUSTOMER_ID and MERCHANT_INVOICE_ID AND PAYINFO_TRANSACT_ID. 
	//
	  $request = new GatewayRequest(); 
	  ...
	  $request->Set(GatewayRequest::MERCHANT_CUSTOMER_ID(), $cust_id . '1CTEST');
	  $request->Set(GatewayRequest::MERCHANT_INVOICE_ID(), $inv_id . '1CTEST');
