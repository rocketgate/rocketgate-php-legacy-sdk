...
$request->Set(GatewayRequest::MERCHANT_INVOICE_ID(), $time . '.ParthAuthTest');

$request->Set(GatewayRequest::AMOUNT(), "9.12");
$request->Set(GatewayRequest::CURRENCY(), "USD");

// $request->Set(GatewayRequest::CARDNO(), "4111111111111111");  // normal test card
// $request->Set(GatewayRequest::CARDNO(), "5103221911199245");	 // Mastercard prepaid example
 $request->Set(GatewayRequest::CARDNO(), "4700142323282751");  // Visa prepaid example
 $request->Set(GatewayRequest::PARTIAL_AUTH_FLAG(), "TRUE");	// Enable Partial Auth
 $request->Set(GatewayRequest::MERCHANT_ACCOUNT(), "2");		// Dev account has many processors, for testing set to a processor that supports Partial Auth

$request->Set(GatewayRequest::EXPIRE_MONTH(), "02");
$request->Set(GatewayRequest::EXPIRE_YEAR(), "2026");
...
if ($service->PerformAuthOnly($request, $response)) {
  ...
  print "Settled Amount: " .  $response->Get(GatewayResponse::SETTLED_AMOUNT()) . "\n";
  print "Balance Amount: " .  $response->Get(GatewayResponse::BALANCE_AMOUNT()) . "\n\n";
