 ...
 * Subsequently, Update Site ID on subscription
 */
...
if ($service->PerformPurchase($request, $response)) {
  ...
  print "GUID: " . $response->Get(GatewayResponse::TRANSACT_ID()) . "\n";
	...
	//
     ...
     // Pass the new Merchant Site ID. Using "2" as an example
     $request->Set(GatewayRequest::MERCHANT_SITE_ID(), "2");

	  if ($service->PerformRebillUpdate($request, $response)) {

        print "PerformRebillUpdate() Succeded\n";
  		  print "New Site ID: " . $response->Get(GatewayResponse::MERCHANT_SITE_ID()) . "\n";

	  } else {
        print "Update Site ID Failed\n";
